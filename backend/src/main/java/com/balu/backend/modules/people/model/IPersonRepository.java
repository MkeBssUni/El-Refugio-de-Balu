...
public interface IPersonRepository extends JpaRepository<Person,Long> {
    ...
    @Query(value = "select concat(p.name, ' ', p.sur_name, ' ', p.last_name) as full_name, p.phone_number, u.username as email from people p join users u on p.user_id = u.id where u.id = ?1;", nativeQuery = true)
    IContactInfoView findContactInfoByUserId(Long userId);
}
