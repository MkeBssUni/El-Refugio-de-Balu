...
public class PersonController {
    ...
    public ResponseEntity<ResponseApi<Boolean>> updatePersonalInfo(@RequestBody UpdatePersonalInfoDto dto) throws Exception{
        try{
            ResponseApi<Boolean> response = personService.updatePersonalInfo(dto);
            return new ResponseEntity<>(response, response.getStatus());
        }catch (Exception e){
            return new ResponseEntity<>(new ResponseApi<>(HttpStatus.INTERNAL_SERVER_ERROR, true, ErrorMessages.INTERNAL_ERROR.name()),HttpStatus.INTERNAL_SERVER_ERROR);
        }
